...
ASSESSMENT_RELATIVE_PATH = Path("rsfc_output/rsfc_assessment.json")
...
def build_rsfc_repository_paths(base_dir: str | Path,target: str,repository_url: str,) -> RepositoryPaths:
    repository = parse_github_repository_url(repository_url)
    project_dir = Path(base_dir) / "outputs" / "rsfc" / target
...
def validate_staged_rsfc_assessment(staging_dir: str | Path,) -> Path:
    assessment_path = (Path(staging_dir) / ASSESSMENT_RELATIVE_PATH)

    if not assessment_path.is_file():
        raise RuntimeError(
            "RSFC did not generate "
            "rsfc_output/rsfc_assessment.json"
        )
...
def promote_staged_rsfc_results(staging_dir: str | Path,active_dir: str | Path,) -> Path:
    staging_path = Path(staging_dir)
    active_path = Path(active_dir)

    validate_staged_rsfc_assessment(staging_path)
