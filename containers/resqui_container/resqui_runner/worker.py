...
def resqui_indicators_generation(job_id,target, repo_url,repos_count, base_dir, token, retries):
    # creamos sesion db
        
        while retries < MAX_RETRIES:
            # ejecutamos resqui_runner por cada worker
            response = resqui_runner(base_dir, target, str(repo_url), token)
        ...
        if response.status["status"] == "error":
            ...
            failed_repo_dir = os.path.join(BASE_DIR,"outputs","resqui",target,repo_name)
        ...
        resqui_target_dir = os.path.join(BASE_DIR,"outputs", "resqui", target)
        completed = count_jsons(resqui_target_dir)

        timestamp(f"[RESQUI] {target} Progress: {completed}/{repos_count} repos processed")
...
def process_message(ch, method, properties, body):
    try:
        ...
        if RATE_LIMIT_RESQUI_ENABLED==True:
            wait_for_token(ch)
            
        requeue = resqui_indicators_generation(job_id, target, repo_url,repos_count, BASE_DIR, TOKEN, 0)
